...
public class G6TAdapter : IG6TAdapter
{
    ...
    public async Task<G6TResponse> SendCommandAsync(G6TCommand command, CancellationToken ct = default)
    {
        ...
        _logger.LogDebug("G6T TX: {Frame}", BytesToHex(frame));
        await _port.WriteAsync(frame, ct).ConfigureAwait(false);

        var raw = await _port.ReadFrameAsync(ct).ConfigureAwait(false);
        _logger.LogDebug("G6T RX: {Frame}", BytesToHex(raw));
    ...
    private G6TResponse ParseResponse(byte[] raw)
    {
        if (raw is null || raw.Length < 8)
        {
            throw new InvalidDataException($"Response quá ngắn: {raw?.Length ?? 0} bytes");
        }

        if (raw[0] != FrameHeader[0] || raw[1] != FrameHeader[1] || raw[2] != FrameHeader[2] || raw[3] != FrameHeader[3])
        {
            throw new InvalidDataException("Header response không hợp lệ.");
        }
        ...
        if (raw[4] != DirReceive)
        {
            throw new InvalidDataException($"Direction byte không hợp lệ: 0x{raw[4]:X2}");
        }
